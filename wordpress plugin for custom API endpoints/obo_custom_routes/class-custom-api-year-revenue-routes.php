...
class OBO_Custom_API_Year_Revenue_Routes extends WP_REST_Controller {
    ...
    public function __construct($api_version, $api_namespace, $json_schema_src) {
        ...
        $this->rest_base = "fiscal-years-revenue/"; // change as needed
    ...
    public function get_year_totals( $request ) {
        ...
        ", 'Revenue', 'Revenue', 1010 );
        $revenue_items = $wpdb->get_results( $query_e );
        // Check to see that there is something to send
        if ( empty($revenue_items) ) {
            return new WP_Error( $this->rest_base, 'No revenue items found', array('status' => 404) );
        }
        // Start preparing response by adding "revenue" object to data array
        foreach ($revenue_items as $row) {
    ...
    public function get_year_dept_totals( $request ) {
        ...
        ", 'Revenue', $fiscal_year_range, 'Revenue', $fiscal_year_range, 1010 );
        $revenue_items = $wpdb->get_results( $query_e );
        // Check to see that there is something to send
        if ( empty($revenue_items) ) {
            return new WP_Error( $this->rest_base, 'No revenue items found', array('status' => 404) );
    ...
    public function prepare_year_totals_for_response( $row, $request ) {
        ...
        $data['general_fund'] = ( ! empty( $schema['properties']['general_fund'] ) ) ?
                ($row->general_fund ? $row->general_fund : '0')
            : '';
        //Set context
        ...
        $schema = array(
            ...
            'properties' => array(
                ...
                'account_description' => array(
                    'description' => __("Account used for a set of budget items. This may often serve as a description of the intended spending target when budget type is Revenue."),
                    'type' => 'string',
                    'context' => array('view', 'embed'),
                    'readonly' => true
                ),
